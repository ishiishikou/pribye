...
- タスク詳細では、期限がないタスクのカレンダー登録ボタンが無効な理由を表示する
...
- プリント詳細では抽出タスク0件時に解析中/解析失敗/タスクなし完了の状態別文言を表示し、解析失敗時の手動登録導線は失敗時アクション側に集約している。
...
- プリント詳細の抽出タスク0件表示を状態別文言にし、解析失敗時に「手動で登録」導線が重複しないようにした。
